...
def parrot_list(request):
    ...
    Retrieves a list of parrots with status set to 'Published'
    from the database.
    **Context**
    ``parrot``
...
def parrot_detail(request, slug):
    ...
    This view retrieves a single parrot with the given slug and status
    set to 'Published' from the database
    **Context**
    ``parrot``
